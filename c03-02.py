...
class SingleLayerNetwork:
    ...
    def prepare_model(self, num_units):
        with tf.name_scope('input'):
            x = tf.placeholder(tf.float32, [None, 784], name='imput')
    ...
    def prepare_session(self):
        ...
        writer = tf.train.SummaryWriter("mnist_sl_logs", sess.graph)
...
for _ in range(4000):
    ...
    if i % 100 == 0:
        summary, loss_val, acc_val = nn.sess.run([nn.summary, nn.loss, nn.accuracy], feed_dict={nn.x: batch_xs, nn.t: batch_ts})
